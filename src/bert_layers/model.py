...
class BertForQuestionAnswering(BertPreTrainedModel):
    """Bert Model with a span classification head for extractive question-answering tasks like SQuAD.
    
    The head is a linear layer that predicts start and end positions of the answer span.
    """

    def __init__(self, config):
        super().__init__(config)
        self.num_labels = 2  # start/end position
        self.config = config
...
class FlexBertForQuestionAnswering(FlexBertPreTrainedModel):
    """Bert Model with a span classification head for extractive question-answering tasks like SQuAD.
    ...
    def _init_weights(self, module: Optional[nn.Module] = None, reset_params: Optional[bool] = None):
        ...
        if module:
            ...
            assert isinstance(reset_params, bool)
            self.bert._init_weights(reset_params=reset_params)
            init_weights(self.config, self.qa_outputs, self.config.hidden_size, type_of_module=ModuleType.final_out)
    ...
    def get_number_parameters(self, count_embeddings: bool = True, trainable: bool = True) -> int:
        ...
        """
        params = self.bert.get_number_parameters(count_embeddings, trainable)
        params += _count_parameters(self.qa_outputs, trainable)
        return params
