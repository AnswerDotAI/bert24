...
class GLiNER(InstructBase):
    def __new__(cls, config):
        if config.span_mode == "token_level":
            ...
            return TokenGLiNER(config, custom_base_model=custom_base_model)
        else:
...
class SpanGLiNER(InstructBase):
    def __init__(self, config):
        ...
        self.token_rep_layer = TokenRepLayer(model_name=config.model_name, fine_tune=config.fine_tune,
                                             ...
                                             custom_base_model=config.custom_base_model,
                                             custom_model_object = config.custom_model_object,
                                             tokenizer_object=config.tokenizer_object)
        

        # Print all the keys in the custom_model_object
        if config.custom_model_object is not None:
            print("Keys in custom_model_object:")
            for key in config.custom_model_object.state_dict().keys():
                print(key)
